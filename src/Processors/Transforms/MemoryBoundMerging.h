...
{
public:
    ParaVal50 pv50 = ParaVal50();
    std::vector<Param> getParaList() override{
        // ParaVal pv50 = ParaVal();
        // vec.push_back(TestC("header", header));
        std::vector<Param> vec;
        vec.push_back(Param("rows",std::to_string(pv50.header.rows())));
        vec.push_back(Param("colomns",std::to_string(pv50.header.columns())));
        vec.push_back(Param("num_inputs",std::to_string(pv50.num_inputs)));
