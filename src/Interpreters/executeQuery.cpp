    ...
    ReadBuffer * istr)
{
    ...
    // location_out << "进入了DB::executeImpl" << endl;
    // location_out.close();
    ...
    try
    {
        if (settings.dialect == Dialect::kusto && !internal) ///非内部查询
        {
    ...
    try
    {
        ...
        /// Put query to process list. But don't put SHOW PROCESSLIST query itself. 子查询也不放进去
        ...
        if (async_insert)
        {
            ...
            if (settings.wait_for_async_insert) //支持异步的插入
            {
